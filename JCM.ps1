...
New-Partition -DiskNumber $disk -Size 1024MB -IsActive  | Format-Volume -FileSystem FAT32 -NewFileSystemLabel 'JCM-Boot'
New-Partition -DiskNumber $disk -UseMaximumSize  | Format-Volume -FileSystem NTFS -NewFileSystemLabel 'JCM-Source'

sleep -Seconds 2

#close new window
foreach ($window in ($shell.Windows() | Where-Object { $_.LocationURL -like "$(([uri]"*JCM-*").AbsoluteUri)*" }))
				{
...
#Finish
Write-host "Your Drive is correctly create, Enjoy"
...
if ((Get-Disk $disk).partitionstyle -eq 'MBR')
				{
                    ...
                    New-Partition -DiskNumber $disk -UseMaximumSize -IsActive | Format-Volume -FileSystem NTFS -NewFileSystemLabel 'JCM-ISO'
