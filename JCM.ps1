...
New-Partition -DiskNumber $disk -Size 1024MB -IsActive  | Format-Volume -FileSystem FAT32 -NewFileSystemLabel 'JMC-Boot'
New-Partition -DiskNumber $disk -UseMaximumSize  | Format-Volume -FileSystem NTFS -NewFileSystemLabel 'JMC-Source'

sleep -Seconds 2

#close new window
foreach ($window in ($shell.Windows() | Where-Object { $_.LocationURL -like "$(([uri]"*JMC-*").AbsoluteUri)*" }))
				{
